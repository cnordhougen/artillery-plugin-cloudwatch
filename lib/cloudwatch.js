...
class CloudWatchPlugin {
    ...
    reportMetrics(report) {
        ...
        // }
        if (latencies.length) {
            latencies.sort((a, b) => a[LatencyDataIndex.TIMESTAMP] - b[LatencyDataIndex.TIMESTAMP]);

            const subSize = latencies.length / 15;
            while (latencies.length) {
                const rawtime = sub[sub.length - 1][LatencyDataIndex.TIMESTAMP]
                    , sub     = latencies.splice(0, subSize)
                                         .map(l => {
                                             l[LatencyDataIndex.LATENCY] /= 1000000;
                                             return l;
                                         })
                                         .sort((a, b) =>
                                             a[LatencyDataIndex.LATENCY] - b[LatencyDataIndex.LATENCY]);
